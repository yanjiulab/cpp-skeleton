...
inline void ConfigCpp::st_impl::handleNotification(const Notification &notification) {
...
inline ConfigCpp::ConfigCpp(int argc, char **argv) : m_pImpl(std::make_unique<ConfigCpp::st_impl>(*this, argc, argv)) {}

inline ConfigCpp::ConfigCpp(int argc, char **argv, std::string &name, std::string &path)
    : m_pImpl(std::make_unique<ConfigCpp::st_impl>(*this, argc, argv, name, std::move(path))) {}

inline ConfigCpp::~ConfigCpp() {
...
inline void ConfigCpp::SetConfigName(std::string name) { m_pImpl->m_name = std::move(name); }

inline void ConfigCpp::SetConfigType(ConfigType type) { m_pImpl->m_type = type; }

inline void ConfigCpp::AddConfigPath(const std::string &path) { m_pImpl->m_path.push_back(std::move(normalizePath(path))); }

inline void ConfigCpp::WatchConfig() {
...
inline void ConfigCpp::OnConfigChange(Callback callback) { m_pImpl->m_callback = std::move(callback); }

inline bool ConfigCpp::ReadInConfig() {
...
inline std::string ConfigCpp::GetConfigData() const {
    if (m_pImpl->m_data) {
...
inline ConfigType ConfigCpp::GetConfigType() const { return m_pImpl->m_type; }

inline bool ConfigCpp::IsSet(const std::string &key) const {
    if (m_pImpl->m_data) {
...
inline bool ConfigCpp::GetBool(const std::string &key) const {
    if (m_pImpl->m_data) {
...
inline int ConfigCpp::GetInt(const std::string &key) const {
    if (m_pImpl->m_data) {
...
inline double ConfigCpp::GetDouble(const std::string &key) const {
    if (m_pImpl->m_data) {
...
inline std::string ConfigCpp::GetString(const std::string &key) const {
    if (m_pImpl->m_data) {
...
inline void ConfigCpp::SetBool(const std::string &key, const bool &boolVal) {
    if (m_pImpl) {
        m_pImpl->m_data->SetBool(key, boolVal);
    }
}

inline void ConfigCpp::SetInt(const std::string &key, const int &intVal) {
    if (m_pImpl) {
        m_pImpl->m_data->SetInt(key, intVal);
    }
}

inline void ConfigCpp::SetDouble(const std::string &key, const double &doubleVal) {
    if (m_pImpl) {
        m_pImpl->m_data->SetDouble(key, doubleVal);
    }
}

inline void ConfigCpp::SetString(const std::string &key, const std::string &stringVal) {
    if (m_pImpl) {
        m_pImpl->m_data->SetString(key, stringVal);
    }
}

inline void ConfigCpp::SetString(const std::string &key, const char *stringVal) {
    if (m_pImpl) {
        m_pImpl->m_data->SetString(key, std::string(stringVal));
    }
}

inline void ConfigCpp::SetDefault(const std::string &key, const bool &boolVal) {
    if (m_pImpl) {
...
inline void ConfigCpp::SetDefault(const std::string &key, const int &intVal) {
    if (m_pImpl) {
...
inline void ConfigCpp::SetDefault(const std::string &key, const double &doubleVal) {
    if (m_pImpl) {
...
inline void ConfigCpp::SetDefault(const std::string &key, const char *stringVal) {
    if (m_pImpl) {
...
inline void ConfigCpp::SetDefault(const std::string &key, const std::string &stringVal) {
    if (m_pImpl) {
...
inline void ConfigCpp::AddBoolOption(const std::string &name, const std::string &helpString) {
    if (m_pImpl) {
        try {
...
inline void ConfigCpp::AddIntOption(const std::string &name, const std::string &helpString) {
    if (m_pImpl) {
        try {
...
inline void ConfigCpp::AddStringOption(const std::string &name, const std::string &helpString) {
    if (m_pImpl) {
        try {
...
inline void ConfigCpp::AddDoubleOption(const std::string &name, const std::string &helpString) {
    if (m_pImpl) {
        try {
...
inline void ConfigCpp::Version(uint32_t &major, uint32_t &minor, uint32_t &patch) {
    major = CONFIG_CPP_VERSION_MAJOR;
    minor = CONFIG_CPP_VERSION_MINOR;
    patch = CONFIG_CPP_VERSION_PATCH;
}

inline std::string ConfigCpp::ConfigFileUsed() const {
    if (m_pImpl) {
        return m_pImpl->m_configFileName;
    }
    return "";
}

inline bool ConfigCpp::WriteConfig() {
    if (!m_pImpl) return false;

    // check file used
    std::ifstream file(m_pImpl->m_configFileName);
    if (!file.good()) {
        std::cerr << "Failed to open file: " << m_pImpl->m_configFileName << std::endl;
        return false;
    }

    // write to file
    std::string ofn = m_pImpl->m_configFileName;
    std::ofstream out_file(ofn);
    if (!out_file.is_open()) {
        std::cerr << "Failed to open file: " << ofn << std::endl;
        return false;
    }

    out_file << GetConfigData();
    out_file.close();
    std::cout << "Successfully saved config to: " << ofn << std::endl;
    return true;
}
}  // namespace ConfigCpp