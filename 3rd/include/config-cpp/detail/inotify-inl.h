#pragma once

#include <fcntl.h>
#include <sys/epoll.h>
#include <sys/stat.h>
#include <sys/types.h>
#include <unistd.h>

#include <cstring>
#include <iostream>
#include <string>
#include <vector>

#include "inotify.h"

namespace ConfigCpp {

static constexpr int PIPE_READ_INDEX = 0;
static constexpr int PIPE_WRITE_INDEX = 1;

inline Inotify::Inotify()
    : m_eventTimeout(0), m_lastEventTime(std::chrono::steady_clock::now()), m_eventMask(IN_ALL_EVENTS), m_epollEvents({}), m_onEventTimeout([](const FileSystemEvent&) {}), m_eventBuffer(MAX_EVENTS * (EVENT_SIZE + EVENT_PAD), 0) {
    constexpr size_t ERR_BUF_SIZE = 256;
    std::array<char, ERR_BUF_SIZE> errbuf{};
    if (pipe2(static_cast<int*>(&m_stopPipeFd[0]), O_NONBLOCK | O_CLOEXEC) == -1) {
        m_error = errno;
        std::stringstream errorStream;
        errorStream << "Can't initialize stop pipe ! " << strerror_r(m_error, errbuf.data(), errbuf.size()) << ".";
        throw std::runtime_error(errorStream.str());
    }

    m_inotifyFd = inotify_init1(IN_NONBLOCK);
    if (m_inotifyFd == -1) {
        m_error = errno;
        std::stringstream errorStream;
        errorStream << "Can't initialize inotify ! " << strerror_r(m_error, errbuf.data(), errbuf.size()) << ".";
        throw std::runtime_error(errorStream.str());
    }

    m_epollFd = epoll_create1(EPOLL_CLOEXEC);
    if (m_epollFd == -1) {
        m_error = errno;
        std::stringstream errorStream;
        errorStream << "Can't initialize epoll ! " << strerror_r(m_error, errbuf.data(), errbuf.size()) << ".";
        throw std::runtime_error(errorStream.str());
    }

    m_inotifyEpollEvent.events = EPOLLIN | EPOLLET;
    m_inotifyEpollEvent.data.fd = m_inotifyFd;
    if (epoll_ctl(m_epollFd, EPOLL_CTL_ADD, m_inotifyFd, &m_inotifyEpollEvent) == -1) {
        m_error = errno;
        std::stringstream errorStream;
        errorStream << "Can't add inotify file descriptor to epoll ! " << strerror_r(m_error, errbuf.data(), errbuf.size()) << ".";
        throw std::runtime_error(errorStream.str());
    }

    m_stopPipeEpollEvent.events = EPOLLIN | EPOLLET;
    m_stopPipeEpollEvent.data.fd = m_stopPipeFd[PIPE_READ_INDEX];
    if (epoll_ctl(m_epollFd, EPOLL_CTL_ADD, m_stopPipeFd[PIPE_READ_INDEX], &m_stopPipeEpollEvent) == -1) {
        m_error = errno;
        std::stringstream errorStream;
        errorStream << "Can't add pipe filedescriptor to epoll ! " << strerror_r(m_error, errbuf.data(), errbuf.size()) << ".";
        throw std::runtime_error(errorStream.str());
    }
}

inline Inotify::~Inotify() {
    stop();

    epoll_ctl(m_epollFd, EPOLL_CTL_DEL, m_inotifyFd, nullptr);
    epoll_ctl(m_epollFd, EPOLL_CTL_DEL, m_stopPipeFd[PIPE_READ_INDEX], nullptr);

    close(m_inotifyFd);
    close(m_epollFd);
    close(m_stopPipeFd[PIPE_READ_INDEX]);
    close(m_stopPipeFd[PIPE_WRITE_INDEX]);
}

inline void Inotify::watchFile(const std::string& filePath) {
    constexpr size_t ERR_BUF_SIZE = 256;
    std::array<char, ERR_BUF_SIZE> errbuf{};
    if (exists(filePath)) {
        m_error = 0;
        int wd = 0;
        wd = inotify_add_watch(m_inotifyFd, filePath.c_str(), m_eventMask);

        if (wd == -1) {
            m_error = errno;
            std::stringstream errorStream;
            if (m_error == ENOSPC) {
                errorStream << "Failed to watch! " << strerror_r(m_error, errbuf.data(), errbuf.size())
                            << ". Please increase number of watches in "
                               "\"/proc/sys/fs/inotify/max_user_watches\".";
                throw std::runtime_error(errorStream.str());
            }

            errorStream << "Failed to watch! " << strerror_r(m_error, errbuf.data(), errbuf.size())
                        << ". Path: " << filePath.c_str();
            throw std::runtime_error(errorStream.str());
        }
        m_directoryMap.insert({wd, filePath});
        m_watchMap.insert({filePath, wd});
    }
}

inline void Inotify::unwatchFile(const std::string& file) {
    removeWatch(m_watchMap[file]);
}

inline void Inotify::removeWatch(int wd) {
    (void)inotify_rm_watch(m_inotifyFd, wd);
}

inline std::string Inotify::wdToPath(int wd) {
    return m_directoryMap[wd];
}

inline void Inotify::onEvent(Event event, EventObserver observer) {
    m_eventMask |= static_cast<std::uint32_t>(event);
    m_eventObserver[event] = std::move(observer);
}

inline void Inotify::onUnexpectedEvent(EventObserver observer) {
    m_unexpectedEventObserver = std::move(observer);
}

inline void Inotify::setEventTimeout(
    std::chrono::milliseconds eventTimeout, std::function<void(FileSystemEvent)> onEventTimeout) {
    m_lastEventTime -= eventTimeout;
    m_eventTimeout = eventTimeout;
    m_onEventTimeout = std::move(onEventTimeout);
}

inline bool Inotify::getNextEvent(FileSystemEvent& event) {
    std::vector<FileSystemEvent> newEvents;

    while (m_eventQueue.empty() && !m_stopped) {
        auto length = readEventsIntoBuffer(m_eventBuffer);
        readEventsFromBuffer(m_eventBuffer.data(), length, newEvents);
        filterEvents(newEvents, m_eventQueue);
    }

    if (m_stopped) {
        return false;
    }

    event = m_eventQueue.front();
    m_eventQueue.pop();
    return true;
}

inline void Inotify::stop() {
    m_stopped = true;
    sendStopSignal();
    if (m_thread.joinable()) {
        m_thread.join();
    }
}

inline void Inotify::start() {
    m_thread = std::thread([this]() { this->run(); });
}

inline void Inotify::run() {
    while (true) {
        if (m_stopped) {
            break;
        }

        FileSystemEvent fileSystemEvent;
        if (!getNextEvent(fileSystemEvent)) {
            continue;
        }

        auto currentEvent = static_cast<Event>(fileSystemEvent.m_mask);
        bool dispatched = false;

        const Notification notification{currentEvent, fileSystemEvent.m_path, fileSystemEvent.m_eventTime};

        for (auto& eventAndEventObserver : m_eventObserver) {
            auto& event = eventAndEventObserver.first;

            if (containsEvent(event, currentEvent)) {
                auto& eventObserver = eventAndEventObserver.second;
                eventObserver(notification);
                dispatched = true;
                break;
            }
        }

        if (!dispatched && m_unexpectedEventObserver) {
            m_unexpectedEventObserver(notification);
        }
    }
}

inline void Inotify::sendStopSignal() {
    std::vector<std::uint8_t> buf(1, 0);
    write(m_stopPipeFd[PIPE_WRITE_INDEX], buf.data(), buf.size());
}

inline bool Inotify::hasStopped() {
    return m_stopped;
}

inline bool Inotify::isOnTimeout(const std::chrono::steady_clock::time_point& eventTime) {
    return std::chrono::duration_cast<std::chrono::milliseconds>(eventTime - m_lastEventTime) < m_eventTimeout;
}

inline ssize_t Inotify::readEventsIntoBuffer(std::vector<uint8_t>& eventBuffer) {
    ssize_t length = 0;
    length = 0;
    auto timeout = -1;
    auto nFdsReady = epoll_wait(m_epollFd, static_cast<epoll_event*>(&m_epollEvents[0]), MAX_EPOLL_EVENTS, timeout);

    if (nFdsReady == -1) {
        return length;
    }

    for (auto n = 0; n < nFdsReady; ++n) {
        if (m_epollEvents[n].data.fd == m_stopPipeFd[PIPE_READ_INDEX]) {
            break;
        }

        length = read(m_epollEvents[n].data.fd, eventBuffer.data(), eventBuffer.size());
        if (length == -1) {
            m_error = errno;
            if (m_error == EINTR) {
                break;
            }
        }
    }

    return length;
}

inline void Inotify::readEventsFromBuffer(
    uint8_t* buffer, ssize_t length, std::vector<FileSystemEvent>& events) {
    ssize_t i = 0;
    while (i < length) {
        auto event = reinterpret_cast<inotify_event*>(&buffer[i]);

        if ((event->mask & IN_IGNORED) != 0) {
            i += EVENT_SIZE + event->len;
            auto filePath = m_directoryMap[event->wd];
            m_directoryMap.erase(event->wd);
            m_watchMap.erase(filePath);

            continue;
        }

        auto path = wdToPath(event->wd);
        if (event->len > 0) {
            path += "/";
            path += std::string(static_cast<char*>(event->name), event->len);
        }

        const FileSystemEvent fsEvent(event->wd, event->mask, path, std::chrono::steady_clock::now());

        if (!fsEvent.m_path.empty()) {
            events.push_back(fsEvent);

        } else {
            // Event is not complete --> ignore
        }

        i += EVENT_SIZE + event->len;
    }
}

inline void Inotify::filterEvents(
    std::vector<FileSystemEvent>& events, std::queue<FileSystemEvent>& eventQueue) {
    for (auto eventIt = events.begin(); eventIt < events.end();) {
        const FileSystemEvent currentEvent = *eventIt;
        if (isOnTimeout(currentEvent.m_eventTime)) {
            eventIt = events.erase(eventIt);
            m_onEventTimeout(currentEvent);
        } else {
            m_lastEventTime = currentEvent.m_eventTime;
            eventQueue.push(currentEvent);
            eventIt++;
        }
    }
}

inline bool Inotify::exists(const std::string& path) {
    struct stat file_stat {};
    return stat(path.c_str(), &file_stat) == 0;
}

}  // Namespace ConfigCpp