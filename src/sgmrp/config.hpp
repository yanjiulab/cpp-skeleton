...
TOML11_DEFINE_CONVERSION_NON_INTRUSIVE(sgmrp::ConfigData::Sub, sub)
TOML11_DEFINE_CONVERSION_NON_INTRUSIVE(sgmrp::ConfigData, dae, port, env, pi, sub)
...
class Config {
  ...
  private:
    explicit Config() : cli_("Lynx Network Daemon") {
        ...
        // config file
        cli_.set_config("--config", "sgmrp.toml", "Read an toml file", true)
