...
DEFINE_MTYPE_STATIC(LIB, HASH, "Hashmap");
DEFINE_MTYPE_STATIC(LIB, HASH_BUCKET, "Hashmap Bucket");
...
struct hashmap* hashmap_new_with_allocator(size_t elsize, size_t cap, uint64_t seed0, uint64_t seed1,
                                           uint64_t (*hash)(const void* item, uint64_t seed0, uint64_t seed1),
                                           int (*compare)(const void* a, const void* b),
                                           void (*elfree)(void* item),
                                           void* udata) {
    ...
    struct hashmap* map = XMALLOC(MTYPE_HASH, size);
    if (!map) {
        return NULL;
    }
    ...
    map->buckets = XMALLOC(MTYPE_HASH_BUCKET, map->bucketsz * map->nbuckets);
    if (!map->buckets) {
        XFREE(MTYPE_HASH, map);
        return NULL;
    }
...
void hashmap_clear(struct hashmap* map, bool update_cap) {
    ...
    if (update_cap) {
        ...
        void* new_buckets = XMALLOC(MTYPE_HASH_BUCKET, map->bucketsz * map->cap);
        if (new_buckets) {
            XFREE(MTYPE_HASH_BUCKET, map->buckets);
...
// hashmap_del_with_hash works like hashmap_del but you provide your
// own hash. The 'hash' callback provided to the hashmap_new function
// will not be called
const void* hashmap_del_with_hash(struct hashmap* map, const void* key,
                                  uint64_t hash) {
    hash = clip_hash(hash);
    map->oom = false;
    size_t i = hash & map->mask;
...
// hashmap_del removes an item from the hash map and returns it. If the
// item is not found then NULL is returned.
const void* hashmap_del(struct hashmap* map, const void* key) {
    return hashmap_del_with_hash(map, key, get_hash(map, key));
}
...
// modified.
//
// Note that if hashmap_del() is called on the hashmap being iterated,
...
static int compare_ints_udata(const void* a, const void* b) {
    return *(int*)a - *(int*)b;
}
...
static void benchmarks(void) {
    ...
    bench("delete", N, {
        const int* v = hashmap_del(map, &vals[i]);
        assert(v && *v == vals[i]);
    }) hashmap_free(map);
