...
#define LIBLINKLIST_VERSION "0.0.1"
...
typedef int (*list_cmp_fn)(const void* val1, const void* val2);
...
/* Indicates that listnode data memory is managed by the list and
 * doesn't need to be freed by app via listnode_delete etc.
 */
#define LINKLIST_FLAG_NODE_DATA_MEM_BY_LIST (1 << 1)
...
/* List is going to manage listnode data memory */
#define listset_list_node_data_mem(X) ((X)->flags |= LINKLIST_FLAG_NODE_DATA_MEM_BY_LIST)
#define listnode_init(X, val)         ((X)->data = (val))
#define listnode_add_tail             listnode_add
#define listnode_get                  listnode_getdata
...
/*
 * Create a new linked list and set node cmp and node data del function
 *
 * Returns:
 *    the created linked list
 */
extern struct list* list_create(int (*cmp)(const void* val1, const void* val2),
                                void (*del)(void* val));
 ...
 *
 * If non-null and set LINKLIST_FLAG_NODE_DATA_MEM_BY_LIST flag list->del is called.
 *
 * Runtime is O(N).
 *
 * list
 ...
 */
extern bool list_scan(struct list* list, bool (*iter)(const void* item, void* udata), void* udata);
