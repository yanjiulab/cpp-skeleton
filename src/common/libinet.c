...
bool is_bcast_mac(const ethaddr_t* mac) {
    ...
    for (i = 0; i < ETH_ALEN; i++)
        if (mac->ether_addr_octet[i] != 0xFF)
            return false;
...
bool is_mcast_mac(const ethaddr_t* mac) {
    if ((mac->ether_addr_octet[0] & 0x01) == 0x01)
        return true;
...
int ether_str2mac(const char* str, ethaddr_t* mac) {
    ...
    if (!mac)
        return 1;
    for (i = 0; i < 6; ++i)
        mac->ether_addr_octet[i] = a[i] & 0xff;
    return 1;
}
...
char* ether_mac2str(const ethaddr_t* mac, char* buf, int size) {
    ...
    snprintf(ptr, ETH_ASLEN, "%02x:%02x:%02x:%02x:%02x:%02x",
             (uint8_t)mac->ether_addr_octet[0], (uint8_t)mac->ether_addr_octet[1],
             (uint8_t)mac->ether_addr_octet[2], (uint8_t)mac->ether_addr_octet[3],
             (uint8_t)mac->ether_addr_octet[4], (uint8_t)mac->ether_addr_octet[5]);
    return ptr;
}
...
int ipaddr_length(int address_family) {
    ...
    default:
        printe("ipaddr_length: bad address family: %d\n",
                  address_family);
        break;
    }
...
int sockaddr_length(int address_family) {
    ...
    default:
        printe("sockaddr_length: bad address family: %d\n",
                  address_family);
        break;
    }
    return 0; /* not reached */
...
void ip_to_sockaddr(const struct ipaddr* ip, uint16_t port,
    ...
    case AF_INET6:
        ...
        break;
    default:
        printe("ip_to_sockaddr: bad address family: %d\n",
                  ip->address_family);
        break;
    }
}
...
void ip_from_sockaddr(const struct sockaddr* address, socklen_t length,
    ...
    case AF_INET6:
        ...
        break;
    default:
        printe("ip_from_sockaddr: bad address family: %d\n",
                  address->sa_family);
        break;
    }
}
