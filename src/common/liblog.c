...
void logger_set_level_by_str(logger_t* logger, const char* szLoglevel) {
    ...
    if (strcasecmp(szLoglevel, "VERBOSE") == 0) {
        loglevel = LOG_LEVEL_VERBOSE;
    } else if (strcasecmp(szLoglevel, "DEBUG") == 0) {
        loglevel = LOG_LEVEL_DEBUG;
    } else if (strcasecmp(szLoglevel, "INFO") == 0) {
        loglevel = LOG_LEVEL_INFO;
    } else if (strcasecmp(szLoglevel, "WARN") == 0) {
        loglevel = LOG_LEVEL_WARN;
    } else if (strcasecmp(szLoglevel, "ERROR") == 0) {
        loglevel = LOG_LEVEL_ERROR;
    } else if (strcasecmp(szLoglevel, "FATAL") == 0) {
        loglevel = LOG_LEVEL_FATAL;
    } else if (strcasecmp(szLoglevel, "SILENT") == 0) {
...
void logger_fsync(logger_t* logger) {
    mutex_lock(&logger->mutex_);
    if (logger->fp_) {
        fflush(logger->fp_);
    }
    mutex_unlock(&logger->mutex_);
}
...
static void lognet_write(logger_t* logger, const char* buf, int len) {
    if (logger->net_fd > 0) {
       write(logger->net_fd, buf, len);
    }
}
...
#undef XXX
    ...
    if (*p) {
        while (*p) {
            if (*p == '%') {
                ...
                case 'l':
                    if (logger->enable_color) {
                        len += snprintf(buf + len, bufsize, "%s", pcolor);
                    }
                    buf[len++] = *plevel;
                    if (logger->enable_color) {
                        len += snprintf(buf + len, bufsize, "%s", CLR_CLR);
                    }
                    break;
                case 'L':
                    if (logger->enable_color) {
                    ...
                    // len += snprintf(buf + len, bufsize, "%s", plevel);
                    for (int i = 0; i < 5; ++i) {
                        buf[len++] = plevel[i];
                    }
                    if (logger->enable_color) {
    ...
    // if (logger->enable_color) {
    //     len += snprintf(buf + len - 1, bufsize - len, "%s\n", CLR_CLR);
    // }
...
void file_logger(int loglevel, const char* buf, int len) {
    logfile_write(default_logger(), buf, len);
}

void net_logger(int loglevel, const char* buf, int len) {
    lognet_write(default_logger(), buf, len);
}

#include <syslog.h>
void syslog_logger(int loglevel, const char* buf, int len) {

    if (loglevel == LOG_LEVEL_SILENT) return;
    // convert loglevel to syslog level
    int syslevel[] = {LOG_DEBUG, LOG_DEBUG, LOG_INFO,
     LOG_WARNING, LOG_ERR, LOG_CRIT};
    syslog(syslevel[loglevel], "%s", buf);
}